...
function expandTourContent(entry: any): Record<string, any> {
  ...
  return {
    ...
    min_participants: Number(c.min_participants) || 0,
    max_participants: Number(c.max_participants) || 0,
    min_age: c.min_age != null ? Number(c.min_age) : null,
    max_age: c.max_age != null ? Number(c.max_age) : null,
    languages: splitLines(c.languages),
    ...
    itinerary: parseJsonSafe<any[]>(c.itinerary, []),
    meeting_points: parseJsonSafe<any[]>(c.meeting_points, []),
...
async function syncTourToResource(tour: ReturnType<typeof expandTourContent>): Promise<string | null> {
  ...
  const resourceContent = {
    ...
    capacity: tour.max_participants || 1,
