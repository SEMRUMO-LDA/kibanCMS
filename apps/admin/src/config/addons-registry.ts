...
const newsletter: AddonDefinition = {
  id: 'newsletter',
  name: 'Newsletter',
  description: 'Capture subscribers and sync them automatically to Brevo (formerly Sendinblue)',
  longDescription: 'Collect newsletter subscribers from any website form. Each new subscriber is stored in the CMS for backup/audit and pushed to Brevo (brevo.com) where the client creates and sends professional campaigns. The CMS handles capture; Brevo handles delivery, templates, A/B testing, and analytics. Free Brevo tier allows 300 emails/day, no credit card. Includes test connection, manual re-sync of existing subscribers, and per-subscriber sync status (success/error/retry).',
  icon: 'mail',
  color: '#2563eb',
  category: 'marketing',
  version: '2.0.0',
  author: 'kibanCMS',
  settingsRoute: '/addons/newsletter',
  collections: [
    {
      name: 'Newsletter Subscribers',
      slug: 'newsletter-subscribers',
      description: 'Email subscribers captured from your website (synced to Brevo when configured)',
      type: 'custom',
      fields: [
        ...
        // Brevo sync metadata
        { id: 'synced_to_brevo', name: 'synced_to_brevo', label: 'Synced to Brevo', type: 'boolean', helpText: 'True when contact was successfully pushed to Brevo' },
        { id: 'brevo_contact_id', name: 'brevo_contact_id', label: 'Brevo Contact ID', type: 'text', helpText: 'Numeric ID returned by Brevo after sync' },
        { id: 'sync_error', name: 'sync_error', label: 'Last Sync Error', type: 'text', helpText: 'Empty when sync succeeded; populated with error message on failure' },
        { id: 'sync_attempts', name: 'sync_attempts', label: 'Sync Attempts', type: 'number', helpText: 'Number of times sync was attempted (auto-incremented)' },
        { id: 'last_synced_at', name: 'last_synced_at', label: 'Last Synced At', type: 'date' },
      ],
    },
  ],
};
...
const stripePayments: AddonDefinition = {
  ...
  collections: [
    ...
    },
    {
      name: 'Orders',
      slug: 'orders',
      description: 'Unified checkout orders — supports bookings, products, and custom line items',
      type: 'custom',
      fields: [
        { id: 'order_number', name: 'order_number', label: 'Order #', type: 'text', required: true, helpText: 'Short human-readable identifier (e.g. ORD-A7K3M2)' },
        ...
        ], helpText: 'Lifecycle state of the order' },
        { id: 'customer_name', name: 'customer_name', label: 'Customer Name', type: 'text', required: true },
        { id: 'customer_email', name: 'customer_email', label: 'Customer Email', type: 'email', required: true },
        { id: 'customer_phone', name: 'customer_phone', label: 'Customer Phone', type: 'text' },
        { id: 'line_items', name: 'line_items', label: 'Line Items (JSON)', type: 'textarea', required: true, helpText: 'JSON array of items: [{"type":"booking|product|custom","label":"...","amount":4500,...}]' },
        { id: 'subtotal', name: 'subtotal', label: 'Subtotal (cents)', type: 'number', required: true },
        { id: 'discount_amount', name: 'discount_amount', label: 'Discount (cents)', type: 'number' },
        { id: 'total', name: 'total', label: 'Total (cents)', type: 'number', required: true, helpText: 'Final amount charged' },
        { id: 'currency', name: 'currency', label: 'Currency', type: 'text', required: true, placeholder: 'eur' },
        { id: 'coupon_code', name: 'coupon_code', label: 'Coupon Code', type: 'text' },
        { id: 'stripe_session_id', name: 'stripe_session_id', label: 'Stripe Session', type: 'text' },
        { id: 'stripe_payment_intent', name: 'stripe_payment_intent', label: 'Payment Intent', type: 'text' },
        { id: 'related_booking_ids', name: 'related_booking_ids', label: 'Related Bookings (JSON)', type: 'textarea', helpText: 'JSON array of booking entry IDs created from line items of type=booking' },
        { id: 'metadata', name: 'metadata', label: 'Metadata (JSON)', type: 'textarea', helpText: 'Custom data passed in by the frontend' },
        { id: 'paid_at', name: 'paid_at', label: 'Paid At', type: 'date' },
        { id: 'cancelled_at', name: 'cancelled_at', label: 'Cancelled At', type: 'date' },
      ],
    },
  ...
  collections: [
    {
      ...
      fields: [
        ...
        { id: 'google_translate_api_key', name: 'google_translate_api_key', label: 'DeepL API Key', type: 'text', required: true, helpText: 'Your DeepL API key (per-client). Free keys end with ":fx". Get one at deepl.com/pro-api' },
