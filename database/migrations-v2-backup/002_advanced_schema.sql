...
DO $$ BEGIN CREATE POLICY "Editors can create content" ON content_nodes FOR INSERT TO authenticated WITH CHECK (organization_id IN (SELECT organization_id FROM profiles WHERE id = auth.uid() AND role::text IN ('super_admin', 'admin', 'editor', 'author'))); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE POLICY "Authors can edit their content" ON content_nodes FOR UPDATE TO authenticated USING (created_by = auth.uid() OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND organization_id = content_nodes.organization_id AND role::text IN ('super_admin', 'admin', 'editor'))); EXCEPTION WHEN duplicate_object THEN null; END $$;
...
DO $$ BEGIN CREATE POLICY "Users can upload to their org" ON media_assets FOR INSERT TO authenticated WITH CHECK (organization_id IN (SELECT organization_id FROM profiles WHERE id = auth.uid() AND role::text IN ('super_admin', 'admin', 'editor', 'author'))); EXCEPTION WHEN duplicate_object THEN null; END $$;
